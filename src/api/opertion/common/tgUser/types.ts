...
export interface MassSendItemV1 {
  ...
  id: number // 唯一标识
  inner_buttons: InnerButton[][] // 内联按钮二维布局
  kind: number // 信息类别：1-只发一次，2-周期发送
  ok_num: number // 成功数量
  period: number | null // 间隔发送周期，以小时为最小单位
  send_at: string | number // 发送时间
  sent_id: number // 发送ID
...
export interface SendGroupMessageParamsV1 {
  ...
  file?: string // 单个文件URL
  chat_ids?: number[] // 聊天/群组ID数组（有选择时传递）
  inner_buttons: number[][] // 内联按钮二维布局（必填，可以为空数组）
