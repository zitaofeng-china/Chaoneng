...
import { v1SendGroupMessage, type SendGroupMessageParamsV1 } from '@/api/management/common/tgUser'
import { v1GetInnerButtonList, type InnerButtonItem } from '@/api/management/common/menuList'
...
// 导入子组件
import MessageContentEditor from '../../UserList/components/MessageDialog/components/MessageContentEditor.vue'
import FileUploader from '../../UserList/components/MessageDialog/components/FileUploader.vue'
import InlineButtonSelector from '../../UserList/components/MessageDialog/components/InlineButtonSelector.vue'
...
const toButtonId = (item: InlineButtonSelection) => {
  return typeof item === 'number' ? item : Number(item)
}

const isPreviewButton = (button: PreviewButton | null): button is PreviewButton => {
  return button !== null
}
...
const checkList = ref<InlineButtonSelection[]>([])
...
const fetchMenuList = async () => {
  try {
    ...
    menuList.value = []
    ElMessage.error('获取内联菜单失败: ' + getErrorMessage(error, '未知错误'))
  }
}
...
const handleSubmit = async () => {
  ...
  // 群组/频道信息
  const chatTypeLabel = getChatTypeText(props.currentGroup.type, '群组')
  ...
  if (checkList.value.length > 0) {
    previewData.buttons = checkList.value
      .map((item) => {
        const id = toButtonId(item)
        const menu = menuList.value.find((m) => Number(m.id) === id)
        return menu ? { id, text: menu.text || '' } : null
      })
      .filter(isPreviewButton)
  }
...
const handleConfirmSend = async (buttonLayout?: number[][]) => {
  ...
  try {
    ...
    const apiParams: SendGroupMessageParamsV1 = {
      ...
      chat_ids: [String(props.currentGroup.id)],
      tg_user_ids: []
    }
    ...
    ElMessage.error(getErrorMessage(error, '发送消息请求失败，请重试'))
  } finally {
    submitting.value = false
  }
}
