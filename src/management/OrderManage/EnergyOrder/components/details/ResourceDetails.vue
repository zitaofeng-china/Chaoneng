...
const props = withDefaults(defineProps<{ orderData: EnergyOrderDetailResponseV1 | null }>(), {
  orderData: null
})

type ResourceTableSlot = TableSlot<ResourceDetail>
...
const amountColumn: TableColumn = {
  ...
  align: 'center',
  width: 120,
  formatter: (row: ResourceDetail) => formatEnergyAmount(row.amount, '0')
}
...
const lifecycleColumns: TableColumn[] = [
  ...
  },
  {
    ...
    label: '发放时间',
    width: 160,
    formatter: (row: ResourceDetail) => formatTableDateTime(row.delegated_at)
  },
  {
    field: 'recycled_at',
    label: '回收时间',
    width: 160,
    formatter: (row: ResourceDetail) => formatTableDateTime(row.recycled_at)
  }
]
...
const activationColumns: TableColumn[] = [
  ...
  },
  {
    ...
    label: '激活时间',
    width: 160,
    formatter: (row: ResourceDetail) => formatTableDateTime(row.actived_at)
  }
]
...
const fullResourceList = computed<ResourceDetail[]>(() => {
