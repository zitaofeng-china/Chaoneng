...
const demoList: QuickChargeOrder[] = [
  {
    id: 'ORD20260429001',
  ...
  },
  {
    ...
    send_address: 'TQpDemoAddr***A3x9',
    receive_address: 'TRxDemoAddr***K2p8',
    type: '带宽出售',
    order_type_label: '速充订单',
    amount: '800 W 带宽',
    unit_price: '0.015',
...
const filterDemoList = (params: any = {}) => {
  return demoList.filter((item) => {
    ...
    const matchStatus = !params.status || item.status === Number(params.status)
...
const fetchDemoList = async (params: any) => {
  ...
  const filteredList = filterDemoList(params)
  const start = (currentPage - 1) * pageSize
  const list = filteredList.slice(start, start + pageSize)

  handleListMessage(list, !!(params?.keyword || params?.type || params?.status), '速充订单')
  return {
    list,
