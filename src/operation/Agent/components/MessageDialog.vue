...
import { v1SendGroupMessage } from '@/api/opertion/common/tgUser'
import type { SendGroupMessageParamsV1 } from '@/api/opertion/common/tgUser'
...
import { v1GetMessageChatList, type MessageChatItem } from '@/api/opertion/common/message'
...
const props = defineProps({
  ...
  user: {
    type: Object as PropType<MessageDialogUser>,
    default: () => ({})
  },
  botList: {
...
const groupOptions = computed(() => {
  return groupList.value.map((group) => {
    const typeLabel = getChatTypeText(group.type, '群组')
    return {
...
const fetchMenuList = async () => {
  try {
    ...
    menuList.value = []
    ElMessage.error('获取内联菜单失败: ' + getErrorMessage(error))
  }
}
...
const fetchGroupList = async (botId?: number | string) => {
  ...
  try {
    ...
    groupList.value = []
    ElMessage.error('获取聊天列表失败: ' + getErrorMessage(error))
  } finally {
