...
import { v1GetQuickChargeOrderList } from '@/api/quick_charge_order'
import { v1GetMessageBotList, type MessageBotItem } from '@/api/message'
      ...
      default: ({ row }: QuickChargeTableSlot) => {
        ...
        return (
          <ElLink
            type="primary"
            onClick={() =>
              router.push({ path: '/bot_manage/bot_list', query: { name: botUserName } })
            }
          >
            {botUserName}
          </ElLink>
...
const fetchDataWrapper = async (params: QuickChargeSearchParams = {}) => {
  try {
    const response = await v1GetQuickChargeOrderList(buildQuickChargeListParams(params))
...
const handleExport = async () => {
  try {
    await exportTableData<EnergyOrder, QuickChargeSearchParams, EnergyListParams>({
      searchTableRef,
      filename: '速充订单列表',
      fetchData: v1GetQuickChargeOrderList,
